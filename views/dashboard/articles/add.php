            ...
            <fieldset class="d-flex flex-column align-items-center">
                <form action="#" method="post" class="noto-sans w-100" enctype="multipart/form-data">
                    <div class="mb-3">
                        ...
                        <input type="text" name="title" id="title" class="form-control" value="<?=$validTitle??''?>" title="Uniquement des lettres, chiffres, points, et les caractères - / . ( ) °" placeholder="exemple: ''Blind-Test Jul le 30/02/25''" pattern="<?=$this->adaptRegex(REGEX_PRODUCT_NAME)?>" required <?=(isset($disabled)) ? 'disabled' : ''?>>
